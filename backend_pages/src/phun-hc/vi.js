...
const localizedValidationMessages = {
    ...
    min: function ({ name, value, args }) {
        if (Array.isArray(value)) {
            return `Giá trị phải lớn ${args[0]} ${name}.`
        }
        const force = Array.isArray(args) && args[1] ? args[1] : false
        if ((!isNaN(value) && force !== 'length') || force === 'value') {
            return `${s(name)} phải lớn ${args[0]}.`
        }
    ...
    integer: function ({ name }) {
        return `${s(name)} phải là số nguyên.`
    },

    maxValue: function ({name, formValues, args}) {
        let target = args[0], targetValue = formValues[target]
        return `${s(name)} phải nhỏ hơn ${targetValue}`
    },
}
