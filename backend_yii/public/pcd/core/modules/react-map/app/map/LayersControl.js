...
export default class LayersControl extends MapControl {
    ...
    componentWillMount() {
        const {children: _children, filterLayers, ...options} = this.props;

        if (L.control.FilterableLayers && filterLayers) {
            this.leafletElement = L.control.FilterableLayers(undefined, undefined, options);
        } else {
            this.leafletElement = L.control.layers(undefined, undefined, options);
        }
