...
//            return $this->redirect($url);
    ...
    protected function createBieudo($type) {
        ...
        $q0 = (new Query())
            ->select([
                '*',
                'year' => "date_part('year', ngaynhapvien)",
                'weekly' => new Expression("CASE WHEN date_part('week', ngaynhapvien) = 53 AND date_part('month', ngaynhapvien) = 1 THEN 1 ELSE date_part('week', ngaynhapvien) END")
            ])
            ->from('cabenh_sxh')
        ;
        $q = (new Query())
