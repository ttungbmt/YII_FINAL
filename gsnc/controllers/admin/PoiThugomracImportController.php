...
class PoiThugomracImportController extends GsncController
{
...
//                $excel->headers = $header;
    ...
    {
        if ($this->validateModels($models, new PoiThugomrac())) {
    ...
    protected function validateModels(&$data, $thugomrac)
    {
        ...
        $modelsThugomrac = array_fill(0, count($data), $thugomrac);

        if (Model::loadMultiple($modelsThugomrac, $data, '') && Model::validateMultiple($modelsThugomrac)) {

            foreach ($data as $row) {

                $thugomrac = new PoiThugomrac();
                $thugomrac->setAttributes($row);
                if($thugomrac->lng && $thugomrac->lat){
                    $thugomrac->geom = [$thugomrac->lng, $thugomrac->lat];
                }
                $thugomrac->save();
        ...
        $this->data['errors'] = $modelsThugomrac;
